...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  // 2. FUNGSI KIRIM OTP EMAIL (MAGIC LINK / OTP)
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // First, trigger Supabase OTP flow
      ...
      // If OTP was generated by Supabase, send it via Resend
      // Note: Supabase will send the email, but we can also use Resend for custom emails
      
      // Jika sukses mengirim kode, pindah ke langkah input OTP
      setStep('otp');
    } catch (error: any) {
      console.error('Error mengirim OTP:', error.message);
      alert('Gagal mengirim kode OTP: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Kode OTP salah atau kedaluwarsa: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
